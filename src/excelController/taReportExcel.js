...
const getTaReportExcel = async (req, res) => {
    ...
    try {
        ...
        let limit = parseInt(req.body.limit) || taReportArr.length
        ...
        let taReportExcel = paginatedData.map((e, i) => ({
            ...
            'Sub Company': e.employeeSubcompanyName,

            'Branch Name': e.employeeBranchName,

            'Department': e.employeeDepartment,

            'Sub Department': e.employeeSubDepartment,
            ...
            'Total Alloted Credit Limit': e.totalCredit,

            'Total Consumed Credit Limit': e.totalUsedCredit,

            'Total Pending Credit Amount': e.pendingCredit,

            'Mode Of Transport': e.modeOfTransport,

            'Amount': e.amount,

            'Document(₹)': e.documentLink,

            'Approval Status': e.taStatus,

            'Payment Status': e.paymentStatus,

            'Payment Date': e.dateOfPayment,
        ...
        return res.status(200).send({
            ...
            'Total Applied': formatAmount(totalApplied),

            'Total Approved': formatAmount(totalApproved),

            'Total Rejected': formatAmount(totalReject),
...
async function fetchData({ pageNo, limit, year, month }) {
    ...
    try {
        ...
        const config = {
            ...
            url:`${process.env.BASE_URL}/report/getTaReportExel`,

            data: { pageNo, limit, year, month }
...
async function getTaReportExcelSheet(req, res) {
    ...
    try {
       ...
       let year = req.query.year||req.body.year
        ...
        let month = req.query.month||req.body.month
        ...
        res.setHeader('Content-Disposition', `attachment; filename="taReport ${month}-${year}.xlsx"`);
...
module.exports = { getTaReportExcel, getTaReportExcelSheet }