...
const getCostCenterReportExcel = async (req, res) => {

    try {

        let data = req.body

        let { DB_NAME, year, month,divisionName,subCompanyName } = data
        ...
        if(!year){
            ...
            return res.status(400).send({status:false,msg:'plz enter year'})

        }

        if(!month){

            return res.status(400).send({status:false,msg:'plz enter month'})
        ...
        if(!subCompanyName){
            ...
            return res.status(400).send({status:false,msg:'plz enter subCompanyName'})

        }

        if (year && month && divisionName=='developer' && subCompanyName=='Bright Future') {
...
async function fetchData({ year,month,divisionName,subCompanyName }) {
    ...
    try {
        ...
        const config = {
            ...
            // url: 'http://localhost:3000/report/getCostCenterReportExcel',

            url:`${process.env.BASE_URL}/report/getCostCenterReportExcel`,

            data: { year,month,divisionName,subCompanyName}
...
async function createExcelFile(data) {
    ...
    let costCenterReportList = data.costCenterReportList

    let header = Object.keys(costCenterReportList)



    values.push(header)

    // costCenterReportList.forEach(e => {

        let value = Object.values(costCenterReportList)
...
async function getCostCenterReportExcelSheet(req, res) {
    ...
    try {
        ...
        let divisionName = req.query.divisionName

        let subCompanyName = req.query.subCompanyName



        let apiData = await fetchData({ year,month,divisionName,subCompanyName })
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="costCenterReportExcelSheet.xlsx"');
...
module.exports = { getCostCenterReportExcel,getCostCenterReportExcelSheet }