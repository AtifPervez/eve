...
const getExpirationPeriod = async (id, db) => {
...
const getRequestAssetReturnListExcel = async (req, res) => {
    ...
    try {
        ...
        let getData = await db.query('select employeeId as empName ,employeeId as employeeName,subCompanyId,subCompanyId as subCompanyName,branchId,branchId as branchName,departmentId,departmentId as departmentName,subDepartmentId,subDepartmentId as subDepartmentName,categoryId,returnDate , categoryId as catagoryName ,assetId,assetId as assetName,serialNo,amount,amount as amountEdt,serialNo,returnDate,handoverTo,damaged_on,image as imageUpload,returnStatus,returnStatus as status,handoverDate,quantity,type from eve_hrm_employee_asset_return_details where status="A"  ', {
        ...
        await Promise.all(getData.map(async (e) => {
            ...
            e.expirationPeriod = await getExpirationPeriod(e.assetId, db)
            ...
            e.empCode = await getEmpCodeFromEmpId(e.empName, db)
            ...
            e.designationId = await getDesignationIdFromEmpId(e.empName, db)
            ...
            e.taggedResponsiblePerson = await getEmployeeNameById(e.taggedResponsible, db)
        ...
        let allotAssetListExcel = paginatedData.map((e, i) => ({
            ...
            'Expiry Period(In Days)': e.expirationPeriod,

            'Asset Return Type': e.typeNew,

            'Damaged On': e.damaged_on,

            'Images Uploaded': e.returnDate,
...
async function fetchData({ token, pageNo, limit, empCode, empName, subCompanyId, branchId, locationId, departmentId, subDepartmentId, designId, taggedResponsible, handoverTo, assetId, status, categoryId }) {
    ...
    try {
        ...
        const config = {
            ...
            // url: 'http://localhost:3000/employeeMaster/getRequestAssetReturnListExcel',

            url:`${process.env.BASE_URL}/employeeMaster/getRequestAssetReturnListExcel`,



            data: { pageNo, limit, empCode, empName, subCompanyId, branchId, locationId, departmentId, subDepartmentId, designId, taggedResponsible, handoverTo, assetId, status, categoryId }
...
async function getRequestAssetReturnListExcelSheet(req, res) {
    ...
    try {
        ...
        let apiData = await fetchData({
            ...
            token, pageNo, limit, empCode, empName, subCompanyId, branchId, locationId, departmentId, subDepartmentId, designId, taggedResponsible, handoverTo, assetId, status, categoryId
        ...
        res.setHeader('Content-Disposition', `attachment; filename="requestAssetReturn.xlsx"`);
...
module.exports = {
    ...
    getRequestAssetReturnListExcel,

    getRequestAssetReturnListExcelSheet
