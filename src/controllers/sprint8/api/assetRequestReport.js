...
const getAssetRequestReport = async (req, res) => {
    try {
        ...
        let countQuery = await db.query(`
                                    ...
                                    FROM eve_hrm_employee_access_request_asset
                                    WHERE status='A'
        ...
        let searchCountQuery = await db.query(`
                                            SELECT COUNT(*) AS total
                                            FROM eve_hrm_employee_access_request_asset
                                            WHERE status='A'
                                            ...
                                            AND (
                                                ...
                                                OR (eve_hrm_employee_access_request_asset.requestDate BETWEEN :requestDateFrom AND :requestDateTo)
                                                )
        ...
        const totalSearchData = searchCountQuery[0].total

        let limit = parseInt(req.body.limit) || 10;
        let maxPage = Math.ceil(totalSearchData / limit)
...
module.exports = { getAssetRequestReport }
