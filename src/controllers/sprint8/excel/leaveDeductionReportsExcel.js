...
const getLeaveDeductionReportsExcel = async (req, res) => {
    try {
...
async function fetchData({ token, month, year, empCode, empId, subCompanyId, branchId, departmentId, subDepartmentId, designationId }) {
    try {
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/reports/getLeaveDeductionReportsExcel`,
...
async function createExcelFile(data) {
    ...
    const totalData = employee[0]['Deduction Details'];
    ...
    header = header.slice(0, totalStartIndex).concat(['Deduction Details']).concat(new Array(totalData.length - 1));
...
async function getLeaveDeductionReportsExcelSheet(req, res) {
    try {
...
module.exports = { getLeaveDeductionReportsExcel,getLeaveDeductionReportsExcelSheet }