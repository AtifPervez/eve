...
const getSalaryTemplateName = async (id, db) => {
...
const getAmlPayrollExcel = async (req, res) => {
    try {
        ...
        if (!year && !month) {
            return res.status(400).send({ status: false, msg: 'enter the fromDate and toDate' })
        }
    ...
    `, {
        ...
        await Promise.all(getData.map(async (e, i) => {
            ...
            e['Template']=await getTemplateNameByTemplateId(e.salaryTemplateId,db)
            if (e['From Date'] !== '' && e['To Date'] !== '') {
                e['Sal Cal Days'] = moment(e['To Date'], 'DD-MM-YYYY').diff(moment(e['From Date'], 'DD-MM-YYYY'), 'days') + 1
            }
            else {
                e['Sal Cal Days'] = ''
            }
            if(e['Sal Cal Days']!=''){
                e['Sal Cal Days']=e['Sal Cal Days'].toString().padStart(2, '0');
            ...
            if (e.daySummary != null) {
                ...
                e['Pay Days'] = e.daySummary.NetPaidDays
            }
...
async function fetchData({ token, year, month,pageNo,limit, subCompanyId, templateId,employeeBranchId, employeeDepartmentId, subDepartmentId, locationID }) {
    try {
        const config = {
            headers: { 'Content-Type': 'application/json', 'x-cross-auth': token },
            method: 'POST',
            
            url: `${process.env.BASE_URL}/payroll/getAmlPayrollExcel`,
            data: { token, year, month,pageNo,limit, subCompanyId,templateId, employeeBranchId, employeeDepartmentId, subDepartmentId, locationID }
        }
...
async function createExcelFile(data) {
    ...
    worksheet.getColumn(46).width = 40
...
async function getAmlPayrollExcelSheet(req, res) {
    try {
        ...
        let apiData = await fetchData({
            token,year, month,pageNo,limit, subCompanyId, templateId,employeeBranchId, employeeDepartmentId, subDepartmentId, locationID
        })
...
module.exports = { getAmlPayrollExcel,getAmlPayrollExcelSheet }