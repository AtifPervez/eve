...
const getShowcauseLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             from eve_hrm_employee_showcause_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.showcauseNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.showcauseNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                               and (:fromDate is null or date(a.showcauseDate) >= :fromDate)
                                                     and (:toDate is null or date(a.showcauseDate) <= :toDate)
        ...
        let getData = await db.query(
            `
                         select 
                             a.id,
                              a.employeeId,
                               date_format(a.showcauseDate,'%d-%m-%Y') as showcauseDate,
                                a.showcauseNameId,
                                  a.createdByBranch,
                                    b.employeeName,
                                      b.employeeEmail,
        ...
        ({
            'Sl. No.': i + 1,
            'Showcause Date': e.showcauseDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getShowcauseLetterExcel`,
...
async function getShowcauseLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Showcause Letter.xlsx"`);
...
module.exports = { getShowcauseLetterExcel, getShowcauseLetterExcelSheet }