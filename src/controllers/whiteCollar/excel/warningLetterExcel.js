...
const getWarningLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             from eve_hrm_employee_warning_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.warningNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.warningNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                               and (:fromDate is null or date(a.warningDate) >= :fromDate)
                                                     and (:toDate is null or date(a.warningDate) <= :toDate)
        ...
        let getData = await db.query(
            `
                         select 
                             a.id,
                              a.employeeId,
                               date_format(a.warningDate,'%d-%m-%Y') as warningDate,
                                a.warningNameId,
                                  a.createdByBranch,
                                    b.employeeName,
                                      b.employeeEmail,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getWarningLetterExcel`,
...
async function getWarningLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Warning Letter.xlsx"`);
...
module.exports = { getWarningLetterExcel, getWarningLetterExcelSheet }