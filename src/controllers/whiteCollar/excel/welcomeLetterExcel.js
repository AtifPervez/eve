...
const getWelcomeLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            ...
                            from eve_hrm_employee_welcome_details as a
                            left join eve_acc_employee as b on a.employeeId=b.id
                            left join eve_hrm_template_master as c on a.welcomeNameId=c.id
                              ...
                              and (:empCode is null or b.employeeCode=:empCode)
                               and (:employeeId is null or a.employeeId=:employeeId)
                                 and (:mobileNo is null or b.employeeMobile=:mobileNo)
                                     and (:templateName is null or a.welcomeNameId=:templateName)
                                     
                            and (:fromDate is null or date(a.welcomeDate) >= :fromDate)
                               and (:toDate is null or date(a.welcomeDate) <= :toDate)
        ...
        let getData = await db.query(
            `
                            ...
                            a.id,
                            a.employeeId,
                            date_format(a.welcomeDate,'%d-%m-%Y') as welcomeDate,
                            a.welcomeNameId,
                            a.createdByBranch,
                            b.employeeName,
                            b.employeeEmail,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate,mobileNo,templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getWelcomeLetterExcel`,
...
async function getWelcomeLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Welcome Letter.xlsx"`);
...
module.exports = { getWelcomeLetterExcel, getWelcomeLetterExcelSheet }