...
const getIncrementLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             from eve_hrm_employee_increment_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.incrementNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.incrementNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                               and (:fromDate is null or a.incrementDate >= str_to_date(:fromDate,'%d-%m-%Y'))
                                                     and (:toDate is null or a.incrementDate <= str_to_date(:toDate,'%d-%m-%Y'))
        ...
        let getData = await db.query(
            `
                         select 
                             a.id,
                              a.employeeId,
                               date_format(a.incrementDate,'%d-%m-%Y') as incrementDate,
                                a.incrementNameId,
                                  a.createdByBranch,
                                    b.employeeName,
                                      b.employeeEmail,
        ...
        ({
            'Sl. No.': i + 1,
            'Increment Date': e.incrementDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getIncrementLetterExcel`,
...
async function getIncrementLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Increment Letter.xlsx"`);
...
module.exports = { getIncrementLetterExcel, getIncrementLetterExcelSheet }