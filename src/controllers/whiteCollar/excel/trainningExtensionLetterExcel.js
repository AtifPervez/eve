...
const getTrainningExtensionLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             from eve_hrm_employee_training_extension_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.trainingExdNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.trainingExdNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                               and (:fromDate is null or a.trainingExdDate >= str_to_date(:fromDate,'%d-%m-%Y'))
                                                     and (:toDate is null or a.trainingExdDate <= str_to_date(:toDate,'%d-%m-%Y'))
        ...
        let getData = await db.query(
            `
                            ...
                            a.id,
                            a.employeeId,
                            date_format(a.trainingExdDate,'%d-%m-%Y') as trainingExdDate,
                            a.trainingExdNameId,
                            a.createdByBranch,
                            b.employeeName,
                            b.employeeEmail,
        ...
        ({
            'Sl. No.': i + 1,
            'Training Extension Date': e.trainingExdDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate,mobileNo,templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getTrainningExtensionLetterExcel`,
...
async function getTrainningExtensionLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Trainning Extension Letter.xlsx"`);
...
module.exports = { getTrainningExtensionLetterExcel, getTrainningExtensionLetterExcelSheet }