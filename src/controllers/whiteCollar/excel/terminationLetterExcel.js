...
const getTerminationLetterExcel = async (req, res) => {
    try {
        ...
        // let db = sequelize('59')
        let data = req.body
        const { branchName, empCode, email, employeeId, fromDate, toDate, mobileNo,templateName,noticePeriod } = data
        const countQuery = await db.query(
            `
                            select count(*) AS total

                             from eve_hrm_employee_termination_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.templateNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.templateNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)
                                     and (:noticePeriod is null or a.noticePeriod=:noticePeriod)  

                                               and (:fromDate is null or a.terminationApply >= :fromDate)
                                                     and (:toDate is null or a.terminationApply <= :toDate)
        ...
        let getData = await db.query(
            `
                        select 
                            a.id,
                             a.employeeId,
                              date_format(a.terminationApply,'%d-%m-%Y') as terminationApply,
                               a.templateNameId,
        ...
        ({
            'Sl. No.': i + 1,
            'Termination Apply Date': e.terminationApply,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    try {
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getTerminationLetterExcel`,
...
async function getTerminationLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Termination Letter.xlsx"`);
        (await getExcel).write(res)
    } catch (error) {
        return res.status(500).send({ status: false, msg: error.message, err: error.stack })
    }
}
module.exports = { getTerminationLetterExcel, getTerminationLetterExcelSheet }