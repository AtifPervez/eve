...
const getConfidentialityAgreementLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            ...
                            from eve_hrm_employee_confidentiality_agreement_details as a
                            left join eve_acc_employee as b on a.employeeId=b.id
                            left join eve_hrm_template_master as c on a.confAgrNameId=c.id
                              ...
                              and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.confAgrNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                               and (:fromDate is null or a.confAgrDate >= :fromDate)
                                                     and (:toDate is null or a.confAgrDate <= :toDate)
        ...
        let getData = await db.query(
            `
                            ...
                            a.employeeId,
                            a.confAgrNameId,
                            date_format(a.confAgrDate,'%d-%m-%Y') as confidentialityDate ,
                            a.createdByBranch,
                            b.employeeName,
                            b.employeeEmail,
                            b.employeeMobile,
        ...
        ({
            'Sl. No.': i + 1,
            'Confidentiality Date': e.confidentialityDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate,mobileNo,templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getConfidentialityAgreementLetterExcel`,
...
async function getConfidentialityAgreementLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Confidentiality Agreement Letter.xlsx"`);
...
module.exports = { getConfidentialityAgreementLetterExcel, getConfidentialityAgreementLetterExcelSheet }