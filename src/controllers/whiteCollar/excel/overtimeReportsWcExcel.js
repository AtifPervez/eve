...
const getOvertimeReportsWcExcel = async (req, res) => {
    try {
...
async function fetchData({ token, year, month, pageNo, limit, empCode, empId, subCompId, branchId, departmentId, subDepartmentId, locationID }) {
    try {
        const config = {
            ...
            url: `${process.env.BASE_URL}/whiteCollar/getOvertimeReportWcExcel`,
...
async function createExcelFile(data) {
    ...
    employee.forEach((e, i) => {
        ...
        row.push(e['Sl. No.'], e['Employee Name'], e['Employee Code'], e['Sub Company'], e['Branch'], e['Department'], e['Sub Department'],
            // e['Location'], e['Type'],
        )
        e.App.forEach((x) => {
            let data = {
                'Actual': x['actualOtHrs'],
...
async function getOvertimeReportsWcExcelSheet(req, res) {

    try {
        // let token = req.headers["x-cross-auth"]
        let token = req.cookies["x-cross-auth"]
...
module.exports = { getOvertimeReportsWcExcel, getOvertimeReportsWcExcelSheet }
