...
const getInterviewQAListExcel = async (req, res) => {
    try {
        ...
        let data = req.body;
        const {answerName,interviewProcess,questionName,branchName,designation  } = data
        const countQuery = await db.query(
            `
                                        ...
                                        AND (:interviewProcess IS NULL OR interviewProcess=:interviewProcess)
        ...
        await Promise.all(getData.map(async e => {
         e.interviewProcessType=await myFunc.getInterviewStatus(e.interviewProcess,db)
        ...
        const excelData = getData.map(e => ({
            'Sl. No.': e.slno,
            'Interview Process Type': e.interviewProcessType,
...
async function fetchData({ token, pageNo, limit,answerName,interviewProcess,questionName,branchName,designation }) {
    ...
    try {
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getInterviewQAListExcel`,

            data: { token, pageNo, limit,answerName,interviewProcess,questionName,branchName,designation }
...
async function getInterviewQAListExcelSheet(req, res) {
    try {

        let token = req.headers["x-cross-auth"]
        let data = { ...req.query, ...req.body }

        let { pageNo, limit,answerName,interviewProcess,questionName,branchName,designation  } = data

        let apiData = await fetchData({token, pageNo, limit,answerName,interviewProcess,questionName,branchName,designation})

        let getExcel = createExcelFile(apiData)
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', `attachment; filename="Interview Q&A List.xlsx"`);
...
module.exports = { getInterviewQAListExcel,getInterviewQAListExcelSheet }