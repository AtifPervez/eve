...
const getProbationLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             ...
                             from eve_hrm_employee_probation_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.probationNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.probationNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)
                                       

                                               and (:fromDate is null or a.probationDate >= :fromDate)
                                                     and (:toDate is null or a.probationDate <= :toDate)
                         ...
                         select 
                             a.id,
                              ...
                              date_format(a.createdDate,'%d-%m-%Y') as appliedDate,
                               date_format(a.probationDate,'%d-%m-%Y') as probationDate,
                                a.probationNameId,
                                  a.createdByBranch,
        ...
        ({
            'Sl. No.': i + 1,
            'Probation Date': e.probationDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    try {
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getProbationLetterExcel`,
...
async function getProbationLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Probation Letter.xlsx"`);
        (await getExcel).write(res)
    } catch (error) {
        return res.status(500).send({ status: false, msg: error.message, err: error.stack })
    }
}
module.exports = { getProbationLetterExcel, getProbationLetterExcelSheet }