// noClaimLetterHistoryExcel
let sequelize = require('../../../config/db')
const { QueryTypes } = require('sequelize')
const myFunc = require('../../../functions/functions')
const axios = require('axios')
const ExcelJS = require('exceljs')
const getReleaseLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             ...
                             from eve_hrm_employee_release_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.releaseNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.releaseNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)
                                               and (:fromDate is null or a.releaseDate >= :fromDate)
                                                     and (:toDate is null or a.releaseDate <= :toDate)
        ...
        let getData = await db.query(
            `
                        select 
                            a.id,
                             a.employeeId,
                              date_format(a.releaseDate,'%d-%m-%Y') as releaseDate,
                               a.releaseNameId,  
                                  a.createdByBranch,
                                    b.employeeName,
                                      b.employeeEmail,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate, mobileNo, templateName }) {
    try {
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getReleaseLetterExcel`,
...
async function getReleaseLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Release Letter.xlsx"`);
        (await getExcel).write(res)
    } catch (error) {
        return res.status(500).send({ status: false, msg: error.message, err: error.stack })
    }
}
module.exports = { getReleaseLetterExcel, getReleaseLetterExcelSheet }