...
const getAntiLiabilityLetterExcel = async (req, res) => {
    try {
        ...
        const countQuery = await db.query(
            `
                            select count(*) AS total
                             from eve_hrm_employee_anti_liability_claim_details as a
                               left join eve_acc_employee as b on a.employeeId=b.id
                                   left join eve_hrm_template_master as c on a.antLiabilityNameId=c.id
                         ...
                         and (:branchName is null or a.createdByBranch=:branchName)
                                 and (:empCode is null or b.employeeCode=:empCode)
                                      and (:employeeId is null or a.employeeId=:employeeId)
                                          ...
                                          and (:templateName is null or a.antLiabilityNameId=:templateName)
                                            and (:email is null or b.employeeEmail=:email)

                                                  and (:fromDate is null or date(a.antLiabilityDate) >= :fromDate)
                                                     and (:toDate is null or date(a.antLiabilityDate) <= :toDate)
        ...
        let getData = await db.query(
            `
                            ...
                            a.id,
                            a.employeeId,
                            date_format(a.antLiabilityDate,'%d-%m-%Y') as antLiabilityDate,
                            a.antLiabilityNameId,
                            a.createdByBranch,
                            b.employeeName,
                            b.employeeEmail,
                            b.employeeMobile,
        ...
        ({
            'Sl. No.': i + 1,
            'Anti Liability Claim Date': e.antLiabilityDate,
...
async function fetchData({ token, pageNo, limit, branchName, empCode, email, employeeId, fromDate, toDate,mobileNo,templateName }) {
    ...
    try {
        ...
        const config = {
            ...
            method: 'POST',
            url: `${process.env.BASE_URL}/whiteCollar/getAntiLiabilityLetterExcel`,
...
async function getAntiLiabilityLetterExcelSheet(req, res) {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="Anti Liability Letter.xlsx"`);
...
module.exports = { getAntiLiabilityLetterExcel, getAntiLiabilityLetterExcelSheet }