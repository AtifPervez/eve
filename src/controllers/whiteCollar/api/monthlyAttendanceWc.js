...
//API
const getMonthlyAttendanceWc = async (req, res) => {
        ...
        if (pageType == 'all-multiple-approve-attendance') {
            countQuery = await db.query(`
                SELECT COUNT(*) AS total FROM eve_acc_employee AS emp
                 ...
                 LEFT JOIN 
            eve_employee_resignation_list AS res ON 
            (
            emp.id = res.empId 
                   AND 
            res.resignStatus = 'A'
    ...
    `,
        ...
        await Promise.all(getData.map(async (e) => {
            ...
            e.compensatoryOffAct = compensatoryOffAct
            ...
            const comapanyPolicy = await db.query(
                ...
                a.id ,
                b.acceptStatus
                FROM eve_hrm_company_policy AS a
                LEFT JOIN eve_company_policy_accept AS b ON a.id=b.policyId
                 WHERE a.status='A' 
                 AND a.subCompanyId=:subCompanyId`,
                {
                    replacements: { subCompanyId: e.subCompanyId },
                    type: QueryTypes.SELECT
                }
            );
            if (comapanyPolicy.length > 0) {
                let allStatusW = comapanyPolicy.every(x => x.acceptStatus === "0")
                let allStatusA = comapanyPolicy.every(x => x.acceptStatus === "1")
        ...
        for (let i = 0; i < getData.length; i++) {
            ...
            for (let j = 0; j < _appDetailsObj.length; j++) {
                ...
                if (_appDetail.crtDate > moment().format('YYYY-MM-DD')) {
                    _appDetail.checkBoxStatus = false
                }
                ...
                if (dojDateFound === true) {
                    _appDetail.dojStatusCheck = true
                }
                if (value.employeeDoj === _appDetail.crtDate) {
                ...
                if (targetObj) {
                    ...
                    _appDetail.checkInSelfiePathForReact = `https://www.eveserver.ind.in/eve/${targetObj.checkInSelfiePath}/${targetObj.checkInSelfie}`
                    ...
                    if (targetObj.lensDeviceName != "--:undefined") {
                        _appDetail.lensdeviceName = targetObj.lensDeviceName
                    }
                ...
                if (targetObj) {
                    ...
                    if (targetObj.intime !== null && targetObj.outTime !== null) {
                        ...
                        if (value.multipleClockInClockOut === 'yes') {
                            const { clockInTime, clockOutTime, lunchIn, lunchOut, minWorkingHour } = timeSchedule;
                            ...
                            // console.log(multi,targetObj.date);
                            ...
                            let netWorkMillis
                            // console.log(moment.utc(netWorkMillis).format("HH:mm"),targetObj.date);
                            // console.log(validate,targetObj.date);
                    ...
                    if (_appDetail.intime !== '') {
                        if (timeSchedule) {
                            if (moment(_appDetail.intime, 'HH:mm') > (moment(timeSchedule.lateTimeEnd, 'HH:mm'))) {
        ...
        getData.map((e) => {
            ...
            totalPaidLeave += parseFloat(e.totalPaidLeave)
...
module.exports = { getMonthlyAttendanceWc }
