...
const getReturnAssetWagesExcel = async (req, res) => {
    ...
    try {
        ...
        let countQuery = await db.query(`
            ...
            AND  DATE_FORMAT(a.returnDate,'%Y-%m')=:date      
        ...
        let getData = await db.query(`
            ...
            a.amount,
            a.quantity,
            a.type AS assetReturnType,
            DATE_FORMAT(a.damaged_on,'%d-%m-%Y') AS damagedOn,
            a.returnStatus AS approvalStatus,
            DATE_FORMAT(a.handoverDate,'%d-%m-%Y') AS handoverDate
        ...
        let excelData = getData.map((e, i) => ({
            ...
            'Damaged On': e.damagedOn,
            'Approval Status': myFunc.capitalizeFirstLetter(e.returnStatus)
        ...
        const fileName = `returnAssetReport${year}_${month}_${Date.now()}.xlsx`;
...
module.exports = { getReturnAssetWagesExcel }
