...
const getApproveSupervisorExcel = async (req, res) => {
    try {
        ...
        await Promise.all(sqlData.map(async e => {
            ...
            // e['LWP(Absent Days)'] = (parseInt(daySummary.absentDays) - parseInt(daySummary.PaidLeaves))
            ...
            //     // Insert 'WA' immediately after 'SA' if missing
            //     if (key === 'SA' && !e.hasOwnProperty('WA')) {
            //         newEmp['WA'] = '0.00';
            //     }

            //     // Insert others immediately after 'Variable Pay' if missing
            //     if (key === 'Variable Pay') {
            //         if (!e.hasOwnProperty('Washing Allowance')) {
            //             newEmp['Washing Allowance'] = '0.00';
            //         }
            //         if (!e.hasOwnProperty('ESIC(Employer Contribution)')) {
            //             newEmp['ESIC(Employer Contribution)'] = '0.00';
            //         }
            //         if (!e.hasOwnProperty('PF(Employer Contribution)')) {
            //             newEmp['PF(Employer Contribution)'] = '0.00';
            //         }
            //     }
            // });
            //  e = newEmp;
        ...
        //         // Insert 'WA' after 'SA' if missing
        //         if (key === 'SA' && !emp.hasOwnProperty('WA')) {
        //             newEmp['WA'] = '0.00';
        //         }

        //         if (key === 'Variable Pay' && !emp.hasOwnProperty('Washing Allowance')) {
        //             newEmp['Washing Allowance'] = '0.00';
        //         }
        //         if (key === 'Variable Pay' && !emp.hasOwnProperty('ESIC(Employer Contribution)')) {
        //             newEmp['ESIC(Employer Contribution)'] = '0.00';
        //         }
        //         if (key === 'Variable Pay' && !emp.hasOwnProperty('PF(Employer Contribution)')) {
        //             newEmp['PF(Employer Contribution)'] = '0.00';
        //         }
        ...
        let excelData = sqlData.map((e, i) => ({
            'Sl. No.': i + 1,
            'Employe Code': e['Employee Code'],
            'Employee Name': e['Employee Name'],
            'DOJ': e['DOJ'],
            'From Date': e['From Date'],
            'To Date': e['To Date'],
            'Bank A/c No.': e['Bank A/c No.'],
            'Bank Name': e['Bank Name'],
            'Bank IFSC': e['Bank IFSC'],
            ...
            'Location': e['Location'],
            'Total Days': e['Total Days'],
            'Working Days': e['Working Days'],
            'Paid Leaves': e['Paid Leaves'],
            'LWP(Absent Days)': e['LWP(Absent Days)'],
            'Present Days': e['Present Days'],
            'Net Paid Days': e['Net Paid Days'],
            'Basic': e['Basic'],
            'CEA (Children Education Allowance)': e['CEA (Children Education Allowance)']||'',
            'ESIC(Employer Contribution)': e['ESIC(Employer Contribution)'] || '',
            'HRA': e['HRA'],
            'Medical Allowance': e['Medical Allowance']||'',
            'Other Allowance': e['Other Allowance']||'',
            'Over Time': e['Over Time'],
            'PF(Employer Contribution)': e['PF(Employer Contribution)'] || '',
            'SA': e['SA']||'',

            'Variable Pay': e['Variable Pay'] || '',
            'WA': e['WA'] || '',
            'Washing Allowance': e['Washing Allowance'] || '',
            'CTC': e['CTC'] || '',
            'ESIC(Employees Contribution)': e['ESIC(Employees Contribution)'] || '',
            'Labour Welfare Fund': e['Labour Welfare Fund'] || '',
            'penulty': e['penulty'] || '',
            'PF(Employees Contribution)': e['PF(Employees Contribution)'] || '',
            'PT': e['PT'] || '',
            'TDS': e['TDS'] || '',
            'Total Earnings': e['Total Earnings'] || '',


            'Total Deductions': e['Total Deductions'] || '',
            'Payable Gross': e['Payable Gross'] || '',
            'Net Pay': e['Net Pay'],
...
module.exports = { getApproveSupervisorExcel }
