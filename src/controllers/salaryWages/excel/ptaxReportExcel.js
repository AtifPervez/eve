...
const getPtaxReportExcel = async (req, res) => {
    try {
        ...
        await Promise.all(sqlData.map(async (e, i) => {
            e.state=await myFunc.getbranchStateName(e.employeeBranchId,db)
            ...
            let totalDeductionAmount = 0
                    ...
                    let grossWages = parseFloat(salaryTypes.inputGross)

                    Object.values(deductionDetails).forEach(item => {
                        if (item?.salaryLabel === 'PT') {
                            e.Ptax = parseFloat((item?.salaryAmount ?? '0').toString().replace(/,/g, '')) || 0;
            ...
            // e['Ptax'] = Ptax
            totalPtax += e.Ptax
            e['grossWages'] = grossWages
            totalgrossWages += parseFloat(grossWages)
        ...
        ({
            ...
            'Gross Wages': myFunc.formatAmount(e.grossWages),
            'Payable PTax': myFunc.formatAmount(e.Ptax),
        }))
        ...
        const worksheet = workbook.addWorksheet('Ptax Report');
        let values = []
        ...
        let a = headers.indexOf('Gross Wages')
        let b = headers.indexOf('Payable PTax')
        let len = headers.length;
        let row = new Array(len).fill('');
        row[a] = `${myFunc.formatAmount(totalgrossWages)}`
        row[b] = `${myFunc.formatAmount(totalPtax)}`
        ...
        const fileName = `PtaxReport_${month}_${year}_${Date.now()}.xlsx`;
...
module.exports = { getPtaxReportExcel }