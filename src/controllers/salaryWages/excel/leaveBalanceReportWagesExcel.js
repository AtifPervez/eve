...
const getLeaveBalanceReportWagesExcel = async (req, res) => {
    try {
...
`, {
        ...
        let values = [];
        let employee = myFunc.replaceEmptyValues(leaveBalanceExcel)
        let header = Object.keys(employee[0]);
        employee.forEach(e => {
            let value = Object.values(e);
            values.push(value);
        });
        const total = employee[0]['Leave Balance'];
        ...
        header = header.slice(0, totalStartIndex).concat(['Leave Balance']).concat(new Array(total.length - 1));
        ...
        const fileName = `leaveBalanceReport_${year}_${month}_${Date.now()}.xlsx`;
...
module.exports = { getLeaveBalanceReportWagesExcel}